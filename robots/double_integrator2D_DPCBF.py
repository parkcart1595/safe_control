...
import casadi as ca
"""
It is based on the double integrator 2D model and overrides
only the continous and discrete-time CBF funcitions for Dynamic Parabolic CBF (DPCBF) counterparts:
"""

class DoubleIntegrator2D_DPCBF(DoubleIntegrator2D):
    ...
    def agent_barrier(self, X, obs, robot_radius, beta=1.0):
        ...
        p_rel = np.array([[obs[0] - X[0, 0]], 
                        [obs[1] - X[1, 0]]])
        v_rel = np.array([[obs_vel_x - X[2, 0]], 
                        [obs_vel_y - X[3, 0]]])
    ...
    def agent_barrier_dt(self, x_k, u_k, obs, robot_radius, beta=1.0):
        ...
        def h(x, obs, robot_radius, beta=1.0):
            ...
            v_rel = ca.vertcat(obs_vel_x - x[2, 0], obs_vel_y - x[3, 0])
